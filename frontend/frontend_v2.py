...
def visualize_predicted_demand(shapefile_path, predicted_demand):
    ...
    """
    # Load the shapefile and convert to EPSG:4326 for plotting
    gdf = gpd.read_file(shapefile_path).to_crs("epsg:4326")

    if "LocationID" not in gdf.columns:
        raise ValueError("Shapefile must contain a 'LocationID' column to match taxi zones.")

    # Map predicted rides to each taxi zone; use 0 if no prediction is available
...
# Optionally set page configuration
# st.set_page_config(layout="wide")

# --- Adjust Timezone to New York ---
...
# --- Merge taxi zone names into predictions ---
if "zone" in geo_df.columns:
    ...
    predictions.rename(columns={"zone": "name"}, inplace=True)
else:
    st.error("The shapefile does not contain a 'zone' column for taxi zone names.")
...
st.dataframe(top10_df[["pickup_location_id", "name", "predicted_demand"]])

# --- Dropdown Menu for Detailed Prediction ---
selected_location = st.selectbox(
    "Select a location for detailed predictions:", options=top10_df["name"].tolist()
)
filtered_ids = top10_df[top10_df["name"] == selected_location]["pickup_location_id"]
if not filtered_ids.empty:
    selected_location_id = filtered_ids.iloc[0]
    st.subheader(f"Prediction Details for {selected_location}")
    fig = plot_prediction(
    ...
    st.error("No prediction data available for the selected location.")